...
type CarrotState = 'IDLE' | 'MOVING_TO_BAG' | 'CARRYING_BAG';

export function CarrotSH({ position }: { position: [number, number, number] }) {
    const { scene, animations } = useGLTF('/models/Carrot_SH_Walk.glb');
    ...
    const [state, setState] = useState<CarrotState>('IDLE');
    ...
    useFrame(() => {
        ...
        switch (stateRef.current) {
            ...
            case 'MOVING_TO_BAG':
                ...
                api.velocity.set(dirToBag.x * 4.5, -1, dirToBag.z * 4.5);
            ...
            case 'CARRYING_BAG':
                ...
                api.velocity.set(dirToBin.x * 4.5, -1, dirToBin.z * 4.5);
                api.rotation.set(0, Math.atan2(dirToBin.x, dirToBin.z), 0);

                if (distToBin < 3.0) {
                    if (aiSellBag) aiSellBag(heldBagValue);
                    setState('IDLE');
                    setHeldBagValue(0);
                }
                break;
...
useGLTF.preload('/models/Carrot_SH_Walk.glb');
