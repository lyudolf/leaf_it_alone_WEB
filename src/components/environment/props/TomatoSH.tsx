...
type TomatoState = 'IDLE' | 'MOVING_TO_BAG' | 'CARRYING_BAG';

export function TomatoSH({ position }: { position: [number, number, number] }) {
    const { scene, animations } = useGLTF('/models/Tomato_SH_Walk.glb');
    ...
    const [state, setState] = useState<TomatoState>('IDLE');
    ...
    useFrame(() => {
        ...
        switch (stateRef.current) {
            ...
            case 'MOVING_TO_BAG':
                ...
                api.velocity.set(dirToBag.x * 5, -1, dirToBag.z * 5);
...
useGLTF.preload('/models/Tomato_SH_Walk.glb');
